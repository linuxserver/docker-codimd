pipeline {
  ...
  environment {
    ...
    DOCKERHUB_IMAGE = 'linuxserver/codimd'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/codimd'
    PR_DOCKERHUB_IMAGE = 'lspipepr/codimd'
    DIST_IMAGE = 'ubuntu'
    MULTIARCH='true'
    CI='true'
    CI_WEB='true'
  ...
  stages {
    ...
    // If this is a alpine_release build use live docker endpoints
    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=alpine_release -v ${TEMPDIR}:/ansible/jenkins linuxserver/jenkins-builder:latest 
              docker pull linuxserver/doc-builder:latest
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=alpine_release -v ${TEMPDIR}:/ansible/readme linuxserver/doc-builder:latest
              if [ "$(md5sum ${TEMPDIR}/${LS_REPO}/Jenkinsfile | awk '{ print $1 }')" != "$(md5sum Jenkinsfile | awk '{ print $1 }')" ] || [ "$(md5sum ${TEMPDIR}/${CONTAINER_NAME}/README.md | awk '{ print $1 }')" != "$(md5sum README.md | awk '{ print $1 }')" ]; then
                ...
                git --git-dir ${TEMPDIR}/repo/${LS_REPO}/.git checkout -f alpine_release
      ...
      steps {
        sh '''#! /bin/bash
              ...
              if [ "${NEW_PACKAGE_TAG}" != "${PACKAGE_TAG}" ]; then
                ...
                git --git-dir ${TEMPDIR}/${LS_REPO}/.git checkout -f alpine_release
    ...
    stage('Github-Tag-Push-Release') {
      ...
      steps {
        ...
        -d '{"tag":"'${EXT_RELEASE_CLEAN}'-ls'${LS_TAG_NUMBER}'",\
             "object": "'${COMMIT_SHA}'",\
             "message": "Tagging Release '${EXT_RELEASE_CLEAN}'-ls'${LS_TAG_NUMBER}' to alpine_release",\
             "type": "commit",\
        ...
        sh '''#! /bin/bash
              ...
              echo '{"tag_name":"'${EXT_RELEASE_CLEAN}'-ls'${LS_TAG_NUMBER}'",\
                     "target_commitish": "alpine_release",\
